...
namespace gft{
  namespace Image32{
	...
	gft::Error((char*)MSG1, (char*)"ComputeCoG");
    ...
    void SelectLargestComp(sImage32 *bin){
      gft::sAdjRel *A=NULL;
      gft::sImage32 *label=NULL;
      int Lmax;
      int *area=NULL;
      int imax,i,p,n=bin->n;
      
      if(gft::Image32::GetMaxVal(bin)==0)
	return;
      A = gft::AdjRel::Circular(1.0); //1.5
      label = gft::Image32::LabelBinComp(bin, A);
      Lmax = gft::Image32::GetMaxVal(label);
