...
namespace gft{
  namespace Heap32fif_lex{
    ...
    sHeap32fif_lex *Create(int n, float *cost1, int *cost2, float *cost3) {
      sHeap32fif_lex *H = NULL;
      int i;
      
      if (cost1 == NULL || cost2 == NULL || cost3 == NULL) {
	fprintf(stdout,"Cannot create heap without cost map in Heap32fif_lex::Create");
	return NULL;
      }
      
      H = (sHeap32fif_lex *) malloc(sizeof(sHeap32fif_lex));
      if (H != NULL) {
	H->n       = n;
	H->cost1   = cost1;
	H->cost2   = cost2;
	...
	if (H->color == NULL || H->pos == NULL || H->pixel == NULL)
	  gft::Error((char *)MSG1,(char *)"Heap32fif_lex::Create");
	for (i = 0; i < H->n; i++) {
	  H->color[i] = WHITE;
	  //H->pos[i]   = -1;
	  //H->pixel[i] = -1;
	...
	*H = NULL;
    ...
    void Update_MaxPolicy(sHeap32fif_lex *H, int p, float value1, int value2, float value3){
      H->cost1[p] = value1;
      H->cost2[p] = value2;
      H->cost3[p] = value3;
    ...
    void Update_MinPolicy(sHeap32fif_lex *H, int p, float value1, int value2, float value3){
      H->cost1[p] = value1;
      H->cost2[p] = value2;
      H->cost3[p] = value3;
      //if (H->color[p] == BLACK) printf("ferrou\n");
