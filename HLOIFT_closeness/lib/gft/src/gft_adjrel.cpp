...
namespace gft{
  namespace AdjRel{
    ...
    void Destroy(sAdjRel **A){
      ...
      aux = *A;
      if (aux != NULL){
	if (aux->dx != NULL) gft::FreeIntArray(&(aux->dx));//free(aux->dx);
	if (aux->dy != NULL) gft::FreeIntArray(&(aux->dy));//free(aux->dy);
	free(aux);
	*A = NULL;
      }   
    }
	...
	*N = NULL;
    ...
    void Reduce2Boundary(sAdjRel **A, float r){
      ...
      int n, p, k;
      if(A == NULL) return;
      AC = gft::AdjRel::Circular(r);
      I = gft::Image32::Render(*A);
      B = gft::Image32::GetObjBorders(I, AC, true);
      //gft::Image32::Write(I, (char *)"I.pgm");
      //gft::Image32::Write(B, (char *)"B.pgm");
      n = 0;
      for(p = 0; p < I->n; p++){
	if(B->data[p] > 0)
	  n++;
      }
      AB = gft::AdjRel::Create(n);
      //AB->dx[0] = (*A)->dx[0];
      //AB->dy[0] = (*A)->dy[0];
      //k = 1;
      k = 0;
	...
	if(B->data[p] > 0){
      ...
      *A = AB;
      ...
      gft::Image32::Destroy(&B);
      gft::AdjRel::Destroy(&AC);
    }
